...
def run_mdnet_rgbt(rgb_img_list, t_img_list, init_bbox, gt=None, seq='seq_name ex)Basketball', savefig_dir='',
                   display=False):
    ############################################
    ############################################
    ############################################
    ...
    result_bb = np.zeros((len(rgb_img_list), 4))
    result[0] = np.copy(target_bbox)
    result_bb[0] = np.copy(target_bbox)

    iou_result = np.zeros((len(rgb_img_list), 1))

    # execution time array
    exec_time_result = np.zeros((len(rgb_img_list), 1))

    # Init model
    model = DFNet(opts['model_path'])
    # if model._get_name() == 'DFNet':  # TODO temperature is set to 31
    #     model.RGB_layers._modules['0']._modules['0'].attention.temperature = 1
    #     model.T_layers._modules['0']._modules['0'].attention.temperature = 1
    #     model.RGB_layers._modules['1']._modules['0'].attention.temperature = 1
    #     model.T_layers._modules['1']._modules['0'].attention.temperature = 1
    #     model.RGB_layers._modules['2']._modules['0'].attention.temperature = 1
    #     model.T_layers._modules['2']._modules['0'].attention.temperature = 1
    ...
    tic = time.time()
    # Load first image
    cur_image_rgb = Image.open(rgb_img_list[0]).convert('RGB')
    cur_image_rgb = np.asarray(cur_image_rgb)
    cur_image_t = Image.open(t_img_list[0]).convert('RGB')
    cur_image_t = np.asarray(cur_image_t)

    # Draw pos/neg samples
    ishape = cur_image_rgb.shape
    ...
    for bidx in range(0, scene_boxes.shape[0]):
        ...
        cropped_image_rgb, cur_image_var_rgb = img_crop_model.crop_image(cur_image_rgb,
                                                                         np.reshape(scene_boxes[bidx], (1, 4)),
                                                                         crop_img_size)
        cropped_image_rgb = cropped_image_rgb - 128.
        ...
        feat_map = model(R=cropped_image_rgb, T=cropped_image_t, out_layer='conv3')
        ...
        cur_pos_feats = model.roi_pool_model(feat_map, cur_pos_rois)
        ...
        cur_neg_feats = model.roi_pool_model(feat_map, cur_neg_rois)
        ...
        cur_bbreg_feats = model.roi_pool_model(feat_map, cur_bbreg_rois)
        cur_bbreg_feats = cur_bbreg_feats.view(cur_bbreg_feats.size(0), -1).data.clone()
        cur_bbreg_feats_fc4 = model(feat=cur_bbreg_feats.detach().clone(), in_layer='fc4', out_layer='fc5')
        ...
        if bidx == 0:
            ...
            ##bbreg feature
            bbreg_feats = cur_bbreg_feats_fc4
            ...
            ##bbreg feature
            bbreg_feats = torch.cat((bbreg_feats, cur_bbreg_feats_fc4), dim=0)
    ...
    for iidx in range(replicateNum):
        ...
        cur_extra_cropped_image_rgb, _ = img_crop_model.crop_image(cur_image_rgb, np.reshape(extra_scene_box, (1, 4)),
                                                                   extra_crop_img_size)
        cur_extra_cropped_image_rgb = cur_extra_cropped_image_rgb.detach()
        ...
        # cur_extra_cropped_image_4hannel = torch.cat((cur_extra_cropped_image_rgb, cur_extra_cropped_image_t), 1)
        ...
        if iidx == 0:
            extra_cropped_image_rgb = cur_extra_cropped_image_rgb
            extra_cropped_image_t = cur_extra_cropped_image_t
            # extra_cropped_image = cur_extra_cropped_image_4hannel
            ...
            extra_cropped_image_rgb = torch.cat((extra_cropped_image_rgb, cur_extra_cropped_image_rgb), dim=0)
            extra_cropped_image_t = torch.cat((extra_cropped_image_t, cur_extra_cropped_image_t), dim=0)
            # extra_cropped_image_rgb = torch.cat((extra_cropped_image_rgb, cur_extra_cropped_image_4hannel), dim=0)
    ...
    extra_cropped_image_rgb -= 128.
    extra_cropped_image_t -= 128.

    extra_feat_maps = model(R=extra_cropped_image_rgb, T=extra_cropped_image_t, out_layer='conv3')
    # Draw pos/neg samples
    ishape = cur_image_rgb.shape

    extra_pos_feats = model.roi_pool_model(extra_feat_maps, extra_pos_rois)
    extra_pos_feats = extra_pos_feats.view(extra_pos_feats.size(0), -1).data.clone()

    extra_neg_feats = model.roi_pool_model(extra_feat_maps, extra_neg_rois)
    extra_neg_feats = extra_neg_feats.view(extra_neg_feats.size(0), -1).data.clone()
    ##bbreg feat
    extra_bbreg_feats = model.roi_pool_model(extra_feat_maps, extra_bbreg_rois)
    extra_bbreg_feats = extra_bbreg_feats.view(extra_bbreg_feats.size(0), -1).data.clone()
    extra_bbreg_feats_fc4 = model(feat=extra_bbreg_feats.detach(), in_layer='fc4', out_layer='fc5')
    ...
    bbreg_feats = torch.cat((bbreg_feats, extra_bbreg_feats_fc4), dim=0)
    ...
    if display or savefig:
        dpi = 80.0
        figsize = (cur_image_rgb.shape[1] / dpi, cur_image_rgb.shape[0] / dpi)

        fig = plt.figure(frameon=False, figsize=figsize, dpi=dpi)
        ax = plt.Axes(fig, [0., 0., 1., 1.])
        ax.set_axis_off()
        fig.add_axes(ax)
        im = ax.imshow(cur_image_rgb, aspect='auto')
    ...
    for i in range(1, len(rgb_img_list)):
        ...
        tic = time.time()
        # Load image
        cur_image_rgb = Image.open(rgb_img_list[i]).convert('RGB')
        cur_image_rgb = np.asarray(cur_image_rgb)
        cur_image_t = Image.open(t_img_list[i]).convert('RGB')
        cur_image_t = np.asarray(cur_image_t)
        ...
        if padded_scene_box[0] > cur_image_rgb.shape[1]:
            padded_scene_box[0] = cur_image_rgb.shape[1] - 1
        if padded_scene_box[1] > cur_image_rgb.shape[0]:
            padded_scene_box[1] = cur_image_rgb.shape[0] - 1
        ...
        sample_feats = model.roi_pool_model(feat_map, sample_rois)
        sample_feats = sample_feats.view(sample_feats.size(0), -1).clone()
        sample_feats_fc4 = model(feat=sample_feats.detach(), in_layer='fc4', out_layer='fc5')
        sample_scores = model(feat=sample_feats, in_layer='fc4')
        ...
        ## Bbox regression
        if success:
            bbreg_feats = sample_feats_fc4[top_idx, :]
        ...
        if opts['visual_log']:
            if gt is None:
                print("Frame %d/%d, Score %.3f, Time %.3f" % (i, len(rgb_img_list), target_score, spf))
            else:
                print("Frame %d/%d, Overlap %.3f, Score %.3f, Time %.3f" %
                      (i, len(rgb_img_list), overlap_ratio(gt[i], result_bb[i])[0], target_score, spf))
        iou_result[i] = overlap_ratio(gt[i], result_bb[i])[0]

    plt.close()
    fps = len(rgb_img_list) / spf_total
