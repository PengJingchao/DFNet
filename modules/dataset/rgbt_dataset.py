...
class RGBT_Dataset(data.Dataset):
    ...
    def __next__(self):
        ...
        for i, (rgb_img_path, t_img_path, bbox) in enumerate(
            ...
            image_t = Image.open(t_img_path).convert('L')
            image_t = np.asarray(image_t).reshape(np.asarray(image_t).shape[0],
                                                  np.asarray(image_t).shape[1], 1)
            ...
            scenes.append(cropped_image_4channel)
