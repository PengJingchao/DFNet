...
class imgCropper(nn.Module):
    def __init__(self, img_size):
        ...
        self.roi_pool_model = PrRoIPool2D(img_size, img_size, 1.)

    def gpuEnable(self):
        self.roi_pool_model = self.roi_pool_model.cuda()
        self.isCuda = True

    def forward(self, image, roi):
        aligned_image_var = self.roi_pool_model(image, roi)
    ...
    def crop_image(self, image, box, result_size):
        ...
        self.roi_pool_model.pooled_width = result_size[0]
        self.roi_pool_model.pooled_height = result_size[1]
    ...
    def crop_several_image(self, img_list, target_list):
        ...
        torch.cuda.synchronize()
        print
        '10 image stacking time:{}'.format(time.time() - start_time)
