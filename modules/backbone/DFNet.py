...
def show_featuremap(feature, name):
    """
    out=F.conv2d(R, self.conv1weight[0], self.conv1bias[0], stride=2)
    out=self.RGB_layers[0][3](self.RGB_layers[0][2](self.RGB_layers[0][1](out)))
    show_featuremap(out)
    """
    ...
    cv2.imwrite(name + ".png", 255 * feature_show.float().numpy())
    import os
...
class Dynamic_conv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, ratio=0.25, stride=1, padding=0, dilation=1, groups=1,
                 bias=True, K=2, temperature=31, init_weight=True):  # temperature=34
        ...
        self.softmax_attention = []  # TODO DELETE self. of softmax_attention
    ...
    def forward(self, x, common_weight, common_bias):  # 将batch视作维度变量，进行组卷积，因为组卷积的权重是不同的，动态卷积的权重也是不同的
        self.softmax_attention = self.attention(x)
        ...
        aggregate_weight = torch.mm(self.softmax_attention, weight).view(-1, self.in_planes, self.kernel_size,
                                                                    self.kernel_size)
        if self.bias is not None:
            two_bias = torch.cat((common_bias, self.bias), dim=0)
            aggregate_bias = torch.mm(self.softmax_attention, two_bias).view(-1)
...
class DFNet(nn.Module):
    def __init__(self, model_path=None, K=1):
        ...
        self.conv1weight = nn.Parameter(torch.Tensor(1, 96, 3, 7, 7), requires_grad=True)
        self.conv1bias = nn.Parameter(torch.Tensor(1, 96), requires_grad=True)
        ...
        self.RGB_layers = nn.Sequential(nn.ModuleList([Dynamic_conv2d(3, 96, kernel_size=7, stride=2),
                                                       nn.ReLU(),
                                                       LRN(),
        ...
        self.T_layers = nn.Sequential(nn.ModuleList([Dynamic_conv2d(3, 96, kernel_size=7, stride=2),
                                                     nn.ReLU(),
                                                     LRN(),
                                                     nn.MaxPool2d(kernel_size=3, stride=2)
                                                     ]),
    ...
    def build_param_dict(self):
        ...
        self.params['conv1weight'] = self.conv1weight
        self.params['conv1bias'] = self.conv1bias
    ...
    def forward(self, R=None, T=None, feat=None, k=0, in_layer='conv1', out_layer='fc6'):
        ...
        for nameid, (name, module) in enumerate(self.layers.named_children()):
            ...
            if run:
                if name == 'conv1':
                    featR = self.RGB_layers[nameid][0](R, self.conv1weight, self.conv1bias)
                    featR = self.RGB_layers[nameid][1](featR)
                    featR = self.RGB_layers[nameid][2](featR)
                    featR = self.RGB_layers[nameid][3](featR)
                    featT = self.T_layers[nameid][0](T, self.conv1weight, self.conv1bias)
                    featT = self.T_layers[nameid][1](featT)
                    featT = self.T_layers[nameid][2](featT)
                    featT = self.T_layers[nameid][3](featT)
    ...
    def load_model(self, model_path):
        ...
        self.conv1weight.data = states['conv1weight']
        self.conv1bias.data = states['conv1bias']
...
if __name__ == "__main__":
    model = DFNet('/home/pjc/MyProgram/RT-MDNet/models/imagenet-vgg-m.mat')
    print(model.conv1weight.shape)
    print(model.conv1bias.shape)
