...
class Dynamic_conv2d(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, ratio=0.25, stride=1, padding=0, dilation=1, groups=1,
                 bias=True, K=2, temperature=34, init_weight=True):
...
class MDNetRgbtDFNetAttention(nn.Module):
    def __init__(self, model_path=None, K=1):
        super(MDNetRgbtDFNetAttention, self).__init__()
        self.K = K
        ...
        for nameid, (name, module) in enumerate(self.layers.named_children()):
            ...
            if run:
                if name == 'conv1':
                    ...
                    feat = self.channel_attention(feat) * feat
                    feat = self.spatial_attention(feat) * feat
...
if __name__ == "__main__":
    model = MDNetRgbtDFNetAttention('/home/pjc/MyProgram/RT-MDNet/models/imagenet-vgg-m.mat')
