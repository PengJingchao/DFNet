...
class MDNetRgbtPixelLevelFusion(nn.Module):
    def __init__(self, model_path=None, K=1):
        super(MDNetRgbtPixelLevelFusion, self).__init__()
        self.K = K
...
if __name__ == "__main__":
    model = MDNetRgbtPixelLevelFusion('/home/pjc/MyProgram/RT-MDNet/models/imagenet-vgg-m.mat')
    ...
    x = torch.rand(1, 4, 107, 107)
    model.cuda()
    y = model(x.cuda(), out_layer='conv3')
    z = model.roi_pool_model(y, torch.tensor([[0, 0, 0, 10, 10], ]).float().cuda())
    z = z.view(z.size(0), -1)
    z = model(z, in_layer='fc4', out_layer='fc6')
