...
from modules.dataset.rgb_t_dataset_separate import *
...
def train_mdnet():
    ...
    if pretrain_opts['set_type'] == 'RGBT234':
        train_rgbt_img_home = './dataset/RGBT234/'
        train_rgb_data_path = './data/rgbt234_V.pkl'
        train_t_data_path = './data/rgbt234_I.pkl'
    elif pretrain_opts['set_type'] == 'GTOT':
        train_rgbt_img_home = './dataset/GTOT/'
        train_rgb_data_path = './data/gtot_V.pkl'
        train_t_data_path = './data/gtot_I.pkl.pkl'
    elif pretrain_opts['set_type'] == 'RGBT70':
        train_rgbt_img_home = './dataset/RGBT70/'
        train_rgb_data_path = './data/rgbt70_V.pkl'
        train_t_data_path = './data/rgbt70_I.pkl'
    ...
    ## Init dataset ##
    with open(train_rgb_data_path, 'rb') as fp1:
        train_rgb_data = pickle.load(fp1)
    with open(train_t_data_path, 'rb') as fp2:
        train_t_data = pickle.load(fp2)

    train_video_numbers = len(train_rgb_data)
    ...
    for real_video_index, (rgb_tuple, t_tuple) in enumerate(zip(train_rgb_data.items(), train_t_data.items())):
        rgb_seqname, rgb_seq = rgb_tuple
        t_seqname, t_seq = t_tuple
        rgb_img_list = rgb_seq['images']
        rgb_gt = rgb_seq['gt']
        rgb_img_dir = os.path.join(train_rgbt_img_home, rgb_seqname)
        t_img_list = t_seq['images']
        t_gt = t_seq['gt']
        t_img_dir = os.path.join(train_rgbt_img_home, t_seqname)
        dataset[real_video_index] = RGB_T_Dataset(rgb_img_dir, rgb_img_list, t_img_dir, t_img_list, rgb_gt,
                                                 model.receptive_field, pretrain_opts)
    ...
    for epoch in range(pretrain_opts['n_cycles']):
        ...
        for step, real_video_index in enumerate(k_list):
            ...
            print("Epoch %2d, step %2d (real_video_index %2d), LR %.6f, BinLoss %.3f, Prec %.3f, interLoss %.3f, Time %.3f" %
                  (epoch, step, real_video_index, scheduler.get_last_lr()[-1], class_loss.item(), train_prec[real_video_index],
        ...
        if cur_score > best_score:
            ...
            states = {'shared_layers': model.layers.state_dict(), 'RGB_layers': model.RGB_layers.state_dict(),
                      'T_layers': model.T_layers.state_dict(), 'conv1weight': model.conv1weight.data,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-init_model_path", default="./models/imagenet-vgg-m.mat")#"./models/DFNet_pre/rt_mdnet22.pth"
